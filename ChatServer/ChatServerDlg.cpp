...
LPARAM CChatServerDlg::OnAccept(WPARAM wParam, LPARAM lParam) {
	// 클라이언트에서 접속 요청이 왔을 때
	m_strStatus = "접속성공";
...
LPARAM CChatServerDlg::OnReceive(WPARAM wParam, LPARAM lParam) {
	// 접속된 곳에서 데이터가 도착했을 때
	char pTmp[256];
	CString strTmp;
	memset(pTmp, '\0', 256);
	...
	strTmp.Format(_T("%s"), CString(pTmp));
